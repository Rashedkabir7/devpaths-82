...
const Index = () => {
  ...
  // Dummy/mock implementations for missing helpers and data
  // Replace with real logic as needed
  const enrollments: Array<{ id: string; roadmap_id: string }> = [];
  const getTotalTimeSpent = () => 0; // returns minutes
  const getCompletedItemsCount = () => 0;
  const getRoadmapOverallProgress = (roadmapId: string) => 0;
  const enrollInRoadmap = (roadmapId: string) => {};
      ...
      <footer className="border-t border-slate-800/50 bg-slate-900/80 py-12 px-4">
        <div className="container mx-auto">
          ...
          <div className="border-t border-slate-800/50 mt-8 pt-8 text-center text-slate-400">
            <p>&copy; 2025 DevPath. All rights reserved. Built for developers worldwide.</p>
          </div>
        </div>
      </footer>
    </div>
